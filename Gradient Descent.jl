...
$h = XW \Rightarrow \frac{\partial h}{\partial W} =  \frac{\partial }{\partial W} XW   = X$
...
$Loss=\frac{1}{2}(\widehat{y}-y)^{2}\Rightarrow \frac{\partial Loss}{\partial \widehat{y}}=\frac{\partial}{\partial \widehat{y}}(\frac{1}{2}(\widehat{y}-y)^{2})=\widehat{y}-y$
"""
...
$\nabla Loss = \frac{\partial Loss(W)}{\partial W} = \frac{\partial Loss}{\partial \hat{y}} \frac{\partial h}{\partial W} \frac{\partial \hat{y}}{\partial h} = \hat{y}X(1-\hat{y})(\hat{y} - y)$
"""
...
begin
	function compute_gradient(error, y_hat, trainX)
	    ...
	    our predictions
	    """
	    grad = trainX' * (error .* sigmoid_deriv(y_hat))
        return grad
	end
end
...
begin
	...
	M = rand(Normal(), (size(X_train)[2], 1))
	num_epochs=100
	learning_rate=0.1
end
...
begin
	#training model
	W, losses = train(M, X_train, y_train, learning_rate, num_epochs)
...
- Accuracy: 0.996, Accuracy is defined as measure of the frequency for the prediction function is correct. It has the formula: (true positive + true negative)/(true positive + false positive + true negative + false negative)
- Recall:  0.9922178988326849, Recall displays the rate of true positive identified by model. It has the formula: true positive / (true positive + false negative).
- Precision: 1.0, Precision displays the percentage of positive prediction which is true positive. It has the formula:: true positives / (true positive + false positive).
- f1-score: 0.99609375, f1-score balances recall and precision: (Recall * Precision)*2/ (Recall+Precision)
"""
...
# ╔═╡ 75d9b201-e083-453d-8af1-3d5af386ad3a
md"""Now I will check learning_rate1 = 0.05, 0.001, 0.00001"""

# ╔═╡ dda7b1d3-c0de-49c9-b8ec-e72a5571b73a
begin
	#initialize our weight matrix and necessary hyperparameters
	N = rand(Normal(), (size(X_train)[2], 1))
	num_epochs1=100
	learning_rate1=0.001
end

# ╔═╡ 22159f94-0ca4-4943-96d0-d6473c7c64f2
begin
	#training model
	P, losses1 = train(N, X_train, y_train, learning_rate1, num_epochs1)
	#visualiza training process
	plot(1:num_epochs1, losses1, legend=false)
end

# ╔═╡ f9639dc9-cc2d-4e08-98a3-7363350810c2
begin
	preds1 = predict(P, X_test)
	acc1 = accuracy(preds1, reshape(y_test, length(y_test), 1))
	p1 = precision(preds1, reshape(y_test, length(y_test), 1))
	r1 = recall(preds1, reshape(y_test, length(y_test), 1))
	f1score1 = 2*p1*r1/(p1 + r1)
	print("acc: $acc1, precision: $p1, recall: $r1, f1_score: $f1score1\n")
end

# ╔═╡ 4703156e-ef9a-4244-8492-17e8a848ae1d
md"""I realize something when trying model with learning rate = 0.001, the value of
accuary, precision, recall and f1_score are all 1.0 which is perfect and this model matches the data 100%. The explaination for this is that: when choosing learning rate = 0.05(big value), maybe the line fluctuates too fast and strongly which can lead to the unfit situation in some points, ortherwise, when choosing learning rate = 0.0001(small value), maybe the line of data fluctuates gradually and slowly which can not be able to keep fit to the model on time. Specially, with the learning rate = 0.001, everything is matched, balance all the data.
"""
