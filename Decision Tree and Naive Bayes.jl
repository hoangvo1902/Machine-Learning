...
# ╔═╡ ecee0e3a-1a37-4b2a-b220-be553c3af254
begin
	using MLJ
	using MLJBase
end
...
function entropy(counts, n_samples)
    ...
    for i in 1:length(counts)
        if counts[i] > 0
            entropy += (counts[i] / n_samples) * log2(counts[i] / n_samples)
        end
    end
    return -entropy
end
...
function Gauss(std, mean, x)
    ...
    #TODO 
	p = 1 / (std * sqrt(2*π)) * exp(-((x - mean) / std)^2 / 2)
	return p
end
...
function plot_pdf(self::NBGaussian)
    #plot Histogram
    #TODO
    bar(collect(keys(self.hist)), collect(values(self.hist)), xlabel="Class", ylabel="Frequency", legend=false)
end   
...
# ╔═╡ fa6f5606-9cd6-46d8-8f9a-7f8451a05226
function fit_1(self::NBGaussian, X,y)
        ...
        for hypo in range(0, length = n_species)
    		...
    		for i in 1:n
        		...
        		class_name = y[i]
        		hypo_label = hypo
        		if class_name == hypo_label
            		push!(rows, feature_values)
        		end
    		end			
			...
			convert_X = deepcopy(X)
			for i = size(y, 1):-1:1
    			if y[i] != hypo
        			convert_X = convert_X[1:size(convert_X, 1) .!= i, :]
    			end
			end
			Mean[hypo] = Statistics.mean(convert_X, dims=1)[1] 
        	Std[hypo] = Statistics.std(convert_X)	
			# println(convert_X)
		end 	
        self.mean=Mean
        self.std=Std
...
# ╔═╡ 6f8fa638-8d45-4406-8cf4-408dd61d7e98
begin
F1 = multiclass_f1score(pred1, y_test)	
Recall = multiclass_recall(pred1, y_test)
Pre = multiclass_precision(pred1, y_test)
println("F1: ", F1)
println("Recall: ", Recall)
println("Precision: ", Pre)
end

# ╔═╡ 20ab1ca9-319e-4b42-9d77-9fc24eb4450b
md"""
As we can see:
...
All the values of F1, Recall and Precision are always at least 90% of all time so that we can confirm this model is good enough to classify this type of data.
"""
