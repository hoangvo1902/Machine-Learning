...
function compute_average_intensity(x, y)
    ...
    total_intensity = zeros(10)
    images = zeros(10)
    for i = 1:size(x, 1)
        num = y[i] + 1 
        total_intensity[num] += sum(x[i, :])
        images[num] += 1
    end
    #TODO compute average intensity for each label
    for num = 1:10
        if images[num] > 0
            mean_[num] = total_intensity[num]/(images[num] * size(x, 2))
        end
    end
    return mean_
end
...
begin
	...
	train_x_new = cat(intensity, symmetry, dims=2)
	size(train_x_new)
end
...
Z-score normalization formula: (x - μ) / σ

x is the orginal value which is normalized

μ is the mean of population of the x values

σ  is standard deviation of population of the x values

- **TODO: Why should we normalize data?**
Because it can remove duplicates from the dataset, handle outliers, and scale the dataset to the common one, so that it can improve performance
"""
...
function compute_gradient(error, train_x)
    ...
    """
	gradient = transpose(train_x) * error / size(train_x, 1)
    return gradient
end
...
# ╔═╡ 76740043-26b0-4bbe-be2e-15f0c09b57bb
md"""
This accuracy is quite good although I got some troube in processing NaN value.
"""
...
begin
	...
	test_x_new = cat(test_intensity, test_symmetry,dims=2)	
	# normalize test_x_new to form normalized_test_x
	normalized_test_x = (test_x_new .-mean_) ./std_ 	
	# add column `ones` to normalized_test_x
...
Both the results of training set and test set has high accuracy, unfortunately I can not process the NaN value which leads to the wrong precision and f1_score.
"""
